...
# Project imports
from server.campaigns.models import PathRule
from server.logging_cfg import setup_logger
...
CAMPAIGN_PATH_RULES = [
    ...
    },
    {
        ...
        "order": 2,
        "pattern": "^(?P<external_id>\\d+)-(?P<date>\\d{4}-\\d{2}-\\d{2})-(?P<metadata__geo_code>.+)$",  # noqa: E501
        "date_format": None,
        "level": "campaign",
    },
    {
        ...
        "order": 3,
        "pattern": "^(?P<metadata__geo_code>.+)-(?P<date>\\d{2}-[a-zA-Z]{3}-\\d{2})-(?P<metadata__geo_type>.+)$",  # noqa: E501
        "date_format": "%d-%b-%y",
        "level": "campaign",
    },
    {
        ...
        "order": 4,
        "pattern": "^(?P<metadata__geo_code>.+)-(?P<date>\\d{2}-\\d{2}-\\d{2})-(?P<metadata__geo_type>.+)$",  # noqa: E501
        "date_format": None,
        "level": "campaign",
    },
    {
        ...
        "order": 5,
        "pattern": "^(?P<id>\\d{1}\\.\\d{1})-(?P<date>\\d{8})-(?P<metadata__geo_code>.+)$",  # noqa: E501
        "date_format": None,
        "level": "campaign",
    },
]
...
DATA_POINT_PATH_RULES = [
    ...
    },
    {
        ...
        "order": 3,
        "pattern": "^Punto[\\W_]*(?P<order>\\d+)-(?P<metadata__observacion>.+)$",  # noqa: E501
        "date_format": None,
        "level": "data_point",
    },
    {
    ...
    },
    {
        ...
        "order": 6,
        "pattern": "^(?P<metadata__tipo_concentracion>[A-Z0-9]+)-[A-Z](?P<order>\\d+)$",  # noqa: E501
        "date_format": None,
        "level": "data_point",
    },
    {
...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        path_rules = (
            COVERAGE_PATH_RULES + CAMPAIGN_PATH_RULES + DATA_POINT_PATH_RULES
        )
        for path_rule_dict in path_rules:
            obj, created = PathRule.objects.update_or_create(
                name=path_rule_dict["name"], defaults=path_rule_dict
            )
            if created:

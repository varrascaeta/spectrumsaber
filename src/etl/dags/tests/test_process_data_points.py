...
class TestProcessDataPointsDag:
    ...
    def dag(self):
        ...
        from etl.dags.process_data_points import (
    ...
    @patch("server.campaigns.models.Campaign")
    def test_get_campaigns_to_scan_no_force(self, mock_campaign, dag):
        ...
        # Mock context
        with patch(
            "etl.dags.process_data_points.get_current_context"
        ) as mock_context:
            mock_context.return_value = {
                "params": {"coverage_name": "TEST", "force_reprocess": False}
            }
            with patch(
                "etl.dags.process_data_points.get_param_from_context"
            ) as mock_get_param:
    ...
    @patch("server.campaigns.models.DataPoint")

...
# Project imports
from src.campaigns.models import UnmatchedFile
...
def match_new_patterns(sender, instance: PathRule, created, **kwargs):
    unmatched_files = UnmatchedFile.objects.filter(
    ...
    for unmatched in unmatched_files:
        ...
        if attributes:
            ...
            if file:
                logger.info("Created matched file %s for UnmatchedFile %s", file.name, unmatched.name)
                unmatched.delete()
